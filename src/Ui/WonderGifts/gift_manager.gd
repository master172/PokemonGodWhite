...
func start_making_gift(num:int,poke:game_pokemon,uid:String):
	var res :String= create_wonder_gift(poke,uid)
...
func create_wonder_gift(monster_res: Resource, sender_uid: String):
...
func load_wonder_gift(import_path: String, current_player_uid: String):
